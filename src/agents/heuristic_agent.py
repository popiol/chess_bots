...
class HeuristicAgent(TrainableAgent):
    ...
    def _predict(self, fen: str, our_squares: list[str]) -> List[PredictionResult]:
        ...
        for move in board.legal_moves:
            ...
            material_weight = 0.11
            mobility_weight = 0.11
            king_weight = 0.05
            castling_weight = 0.05
            check_weight = 0.03
            undefended_weight = 0.11
            center_weight = 0.05
            undeveloped_weight = 0.05
            doubled_weight = 0.02
            isolated_weight = 0.02
            passed_weight = 0.05
            attacked_weaker_weight = 0.35

            material_eval = self._material_eval(b2, is_white)
            mobility_eval = self._mobility_eval(b2, is_white)
            king_eval = self._king_safety_eval(b2, is_white)
            castling_eval = self._castling_bonus(b2, is_white)
            check_eval = self._check_eval(b2, is_white)
            undefended_eval = self._undefended_attack_eval(b2, is_white)
            center_eval = self._center_control_eval(b2, is_white)
            undeveloped_eval = self._undeveloped_pieces_eval(b2, is_white)
            doubled_eval = self._doubled_pawns_eval(b2, is_white)
            isolated_eval = self._isolated_pawns_eval(b2, is_white)
            passed_eval = self._passed_pawns_eval(b2, is_white)
            attacked_weaker_eval = self._attacked_by_weaker_eval(b2, is_white)

            eval_val = (
                material_weight * material_eval
                + mobility_weight * mobility_eval
                + king_weight * king_eval
                + castling_weight * castling_eval
                + check_weight * check_eval
                + undefended_weight * undefended_eval
                + center_weight * center_eval
                + undeveloped_weight * undeveloped_eval
                + doubled_weight * doubled_eval
                + isolated_weight * isolated_eval
                + passed_weight * passed_eval
                + attacked_weaker_weight * attacked_weaker_eval
            )
            eval_val = float(np.clip(eval_val, -1.0, 1.0))

            # Decisive ratio: weighted sum of absolute metric values (normalized to [0,1])
            decisive_ratio = (
                material_weight * abs(material_eval)
                + mobility_weight * abs(mobility_eval)
                + king_weight * abs(king_eval)
                + castling_weight * abs(castling_eval)
                + check_weight * abs(check_eval)
                + undefended_weight * abs(undefended_eval)
                + center_weight * abs(center_eval)
                + undeveloped_weight * abs(undeveloped_eval)
                + doubled_weight * abs(doubled_eval)
                + isolated_weight * abs(isolated_eval)
                + passed_weight * abs(passed_eval)
                + attacked_weaker_weight * abs(attacked_weaker_eval)
            )
